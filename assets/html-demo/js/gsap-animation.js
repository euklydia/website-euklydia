...
function pbmit_portfolio_style3() {
    // Define a function that sets up the swiper and the scrolling animations
    ...
    ScrollTrigger.matchMedia({
        // All screens (this will include mobile screens)
        "all": setupPortfolio,
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    var boxes = document.querySelectorAll('.pbmit-element-portfolio-style-3 .pbminfotech-box-wrap');
    console.log("Number of boxes:", boxes.length);  // Check how many boxes are found
    if (boxes.length > 0) {
        var firstBoxWidth = boxes[0].offsetWidth;  // Get the width of the first box
        console.log("Width of the first box:", firstBoxWidth);  // Check the width found

        boxes.forEach(function(box, index) {
            box.style.width = firstBoxWidth + 'px';  // Set the width of each box to match the first one
            console.log(`Box ${index} new width:`, box.style.width);  // Log the new width set for each box
        });
    }
});
window.onload = function() {
    var boxes = document.querySelectorAll('.pbmit-element-portfolio-style-3 .pbminfotech-box-wrap');
    if (boxes.length > 0) {
        var firstBoxWidth = boxes[0].offsetWidth;
